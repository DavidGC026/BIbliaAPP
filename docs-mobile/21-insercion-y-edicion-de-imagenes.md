...
| `mobile/app/note/[noteId].tsx` | Puente RN: picker, `sendToEditor`, auto-guardado del HTML |
...
4. Guarda, sal y reabre la nota: la imagen y su formato deben persistir.
5. **Regresión toolbar:** tras editar una imagen, confirma que colores y fuentes siguen operativos (antes del fix de `.join('\\n')` fallaban todos a la vez).

...
- **Móvil web (julio 2026):** cabecera más compacta, métricas de palabras/min ocultas en pantallas `< sm`, modo inmersivo al editar desde la sección Notas (header/tabbar ocultos), altura del editor ligada a `visualViewport` y colapso del bloque de título con teclado abierto (`body.keyboard-open` oculta estado y métricas, ~45 px extra para el editor). Ver [`docs/notas-web-paridad-movil.md`](../docs/notas-web-paridad-movil.md) § *Área de escritura en móvil web*.

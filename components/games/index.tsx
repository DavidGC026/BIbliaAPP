...
import { loadGameContent } from "@/lib/game-api-client"
...
export function BibleGames({ userId, isAdmin = false, onOpenPassage }: { userId?: number; isAdmin?: boolean; onOpenPassage: OpenPassage }) {
  const session = useGamesSession({ storage, storageKey: `biblia-games-v2-${userId ?? "guest"}`, legacyKey: `biblia-games-v1-${userId ?? "guest"}`, loadContent: loadGameContent })
  ...
  const roundProps = { onComplete: session.onComplete, onOpen: onOpenPassage, onRestart: session.restart, settings: active?.settings, onAttempt: session.onAttempt }

  if (view === "editor" && isAdmin) return <div className="mx-auto max-w-4xl space-y-5"><Button variant="outline" className="min-h-11" onClick={() => { setView("free"); void session.refresh() }}>Volver a juegos</Button><ContentEditor /></div>
  return <div className="mx-auto max-w-4xl space-y-7 pb-8 text-foreground">
    {game && active ? <div className="mx-auto max-w-2xl space-y-6">
      ...
      <section className="space-y-4 border-t border-border pt-6" aria-label="Tus resultados">
        ...
        <p className="text-xs text-muted-foreground">Resultados, palabras jugadas y repasos guardados en este navegador{userId ? " para tu cuenta" : " como visitante"}. No se sincronizan entre dispositivos.</p>
      </section>
    </>}

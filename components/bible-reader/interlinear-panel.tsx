...
export const InterlinearPanel = memo(function InterlinearPanel({
  ...
  const sections = active?.definition ? parseStrongDefinition(active.definition) : []
  ...
  return (
    ...
    >
      ...
      >
        <Languages className="size-3.5" style={accentColor ? { color: accentColor } : undefined} />
      ...
      {expanded && (
        <div className="mt-2 space-y-3 pl-1">
          <div
            className={cn("flex flex-wrap gap-x-1 gap-y-2", rtl && "flex-row-reverse")}
            dir={rtl ? "rtl" : "ltr"}
          >
            {words.map((word, index) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "interlinear-word min-w-[3.2rem] rounded-md border px-1.5 py-1 text-center transition-colors",
                    selected
                ...
                >
                  ...
                  {word.transliteration && (
                    <span className="block text-[10px] italic text-muted-foreground" dir="ltr">
                      {word.transliteration}
                    </span>
                  )}
                  <span
          ...
          {active && (
            <aside className="rounded-lg border border-border bg-card/50 p-3" dir="ltr">
              <div className="flex flex-wrap items-baseline gap-2">
                ...
                {active.morph && (
                  <span className="text-[11px] text-muted-foreground">{active.morph}</span>
                )}
              </div>
              {sections.length > 0 ? (
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  {active.definition || "Sin definición Strong para esta forma."}
                </p>
              )}
            </aside>
          )}

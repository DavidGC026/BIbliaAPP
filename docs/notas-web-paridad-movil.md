...
- Los bloques insertados (versículo, diccionario, tabla) viven en `lib/note-editor-blocks.ts`, espejo del módulo móvil. Incluye la reparación de bloques que el motor del `contenteditable` rompía al borrar justo encima o debajo (se perdía la barra de botones y el bloque quedaba inservible) y el borrado en dos pasos: la primera pulsación selecciona el bloque, la segunda lo elimina completo. Detalle y pruebas en [`docs-mobile/29-notas-bloques-de-contenido.md`](../docs-mobile/29-notas-bloques-de-contenido.md); verificación con `node scripts/test_note_blocks.cjs`.

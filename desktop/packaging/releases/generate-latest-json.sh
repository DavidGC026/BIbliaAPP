...
# Uso: ./generate-latest-json.sh 0.3.3 [directorio-artefactos] [salida.json]
# Plantilla sin artefactos: ./generate-latest-json.sh --dry 0.3.3
...
if [[ "${1:-}" == "--dry" ]]; then
  DRY=true
  VERSION="${2:?Versión requerida (ej. 0.3.3)}"
  OUT="${3:-$(dirname "$0")/latest.json}"
  ARTIFACTS_DIR=""
else
  VERSION="${1:?Versión requerida (ej. 0.3.3)}"

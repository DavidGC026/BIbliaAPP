...
export async function setLocalHighlight(
  ...
  if (existing) {
    await run("UPDATE highlights SET color = ?, dirty = 1, deleted = 0 WHERE id = ?", [
      color,
      existing.id,
    ]);
    return;
...
export async function getLocalChapterNotes(
  bookId: number,
  chapter: number,
): Promise<VerseNoteLink[]> {
...
export async function saveLocalVerseNote(
  ...
  if (existing) {
    await run("UPDATE verse_notes SET note_content = ?, dirty = 1 WHERE id = ?", [
      noteContent,
      existing.id,
    ]);
    return existing.id;
...
export async function upsertHighlightsFromServer(
  ...
  for (const h of highlights) {
    ...
    if (existing) {
      await run("UPDATE highlights SET color = ?, deleted = 0, dirty = 0 WHERE id = ?", [
        h.color,
        existing.id,
      ]);
    } else {
...
export async function upsertVerseNotesFromServer(
  _bookId: number,
  _chapter: number,
  links: VerseNoteLink[],
) {

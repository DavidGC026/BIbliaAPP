import { getAll, getFirst, nowIso, run, tempId } from "@/lib/offline/db";
...
export async function updateLocalNotebook(
  id: number,
  name: string,
  coverImage?: string | null,
) {
...
export async function deleteLocalNotebook(id: number) {
  ...
  if (!row) return;
  await run("UPDATE notebooks SET deleted = 1, dirty = 1, updated_at = ? WHERE id = ?", [
    nowIso(),
    row.id,
  ]);
  await run("UPDATE notes SET deleted = 1, dirty = 1, updated_at = ? WHERE notebook_id = ?", [
    nowIso(),
    row.id,
  ]);
}
...
export async function deleteLocalNote(id: number) {
  ...
  if (!row) return;
  await run("UPDATE notes SET deleted = 1, dirty = 1, updated_at = ? WHERE id = ?", [
    nowIso(),
    row.id,
  ]);
}
...
/** ponytail: repara duplicados de libretas; corre en cada sync */
export async function repairNotebookData(): Promise<void> {
  ...
  for (const [serverId, rows] of byServer) {
    ...
    for (const r of rows) {
      ...
      await run("UPDATE notes SET notebook_id = ? WHERE notebook_id = ?", [
        canonical.id,
        r.id,
      ]);
  ...
  for (const nb of fresh) {
    ...
    if (target && target.id !== nb.id) {
      await run("UPDATE notes SET notebook_id = ?, dirty = 1 WHERE notebook_id = ?", [
        target.id,
        nb.id,
      ]);

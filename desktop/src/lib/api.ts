...
export async function searchVerses(bibleId: number, q: string) {
  return request<{ verses: Verse[]; total: number; isReference?: boolean }>(
...
export async function getCrossReferences(
  ...
  return request<{ references: CrossReference[] }>(
...
export async function deleteFavorite(id: number) {
  return request<{ success: boolean }>(`/api/favorites?id=${id}`, {
    method: "DELETE",
  });
}
...
export async function listDevotionals() {
  return request<{ devotionals: Devotional[] }>("/api/devotionals");
}
...
// — Libros de estudio —
function mapExternalBook(row: Record<string, unknown>): ExternalBook {
  return {
    id: row.id as number,
...
function mapBookLog(row: Record<string, unknown>): BookLog {
  return {
    id: row.id as number,
...
export async function likeFeedPost(postId: number) {
  return request<{ success: boolean }>(`/api/feed/posts/${postId}/like`, {
    method: "POST",
  });
}
...
export async function getFeedComments(postId: number) {
  return request<{ comments: FeedComment[] }>(
